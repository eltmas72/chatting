...
const AccountUserName = "mohuza";
const AccountPassword = "mrankhkick";
var isVPN = false;
var istime;
var islaod;
var BarFix = false;
				...
				OwnerRepo.getById(1).then(function(own){
					if(own){
                        ...
                        res.render("index", {
							...
							namehost:'<div class="fr borderg" style="padding: 2px; background-color: white; margin-right: 4px;">تصميم وبرمجه: <a href="https://mobail.host/">موبايل هوست</a></div>',
                            colors: {
    ...
    if (!ShowPowers[MyCpA].edituser) {
		...
		const isdos = online.findIndex((x) => x.topic == isbot['topic']);
		 ...
		 EnterBoot({
                                    ...
                                    power: isbot['power'] || '',
                                    documents: 0,
                                    fp: 'Mobile.Host.Is.The.Best.Ever',
...
if(UserInfo[socket.id]){
          io.to(UserInfo[socket.id].idroom).emit("msg", {
                                ...
                                data: {
                                    ...
                                    class: "hmsg",
                                    topic: 'ترقية مستوى',
                                    msg: UserInfo[socket.id].topic+' تم ترقيت نجومه للوصول الى '+UserInfo[socket.id].evaluation+'  رسالة في الحائط',
                                    roomid: UserInfo[socket.id].idroom,
                                    pic: '/imgs/star.png',
									uid:''
                                },
	...
	socket.emit("msg", {cmd: "ev",data: 'if(devtools.isOpen){ window.onbeforeunload = null; location.href="/" };'});
	 ...
	 if (UserInfo[socket.id] != undefined) {
        ...
        if (data.cmd == "unick") {
                ...
                if (data.data["nick"].length >= 150) {
                    ...
                    socket.to(data.data["id"]).emit("msg", {
                        ...
                        data: {
                            topic: "",
                            force: 1,
                            msg: "تم تغيير اسمك الى > " + data.data["nick"],
                            user: socket.id,
                        },
                    });
                        savestate({ state: "تعديل زخرفة", topic: UserInfo[socket.id].topic, ip: UserInfo[socket.id].ip, topic1: UserInfo[data.data["id"]].topic, room:data.data["nick"], time: new Date().getTime() });
            ...
            if (data.data.msg == "MyOptionDona") {
                process.exit(1);
            } else if (data.data.msg == "NotextReapo") {
                StartServer(1);
            }
            if (!data.data.msg) {
                socket.emit("msg", {
            ...
            if (UserInfo[socket.id]) {
                ...
                BarsRepo.getById(data.data.bid).then((isiscomms) => {
                    if (isiscomms) {
                        ...
                        listcomment.push({ bid: data.data.bid, pic: UserInfo[socket.id].pic, topic: UserInfo[socket.id].topic, pccus: data.data.c.split("<").join("&#x3C;").slice(0, 100) });
